    ...
    for (int i = 0; i < times; i++) {
      ...
      if (commonQuery) {
        ...
        if (Interval.PERIOD.equals(interval)) {
          descriptor = new CommonFormulaQueryDescriptor(TEST_TABLE, s1, s2, event, seg, Filter.ALL, 1d, interval,
                                                        CommonQueryType.NORMAL);
        } else {
          descriptor = new CommonFormulaQueryDescriptor(TEST_TABLE, s1, s1, event, seg, Filter.ALL, 1d, interval,
                                                        CommonQueryType.NORMAL);
        }
      } else {
        descriptor = new GroupByFormulaQueryDescriptor(TEST_TABLE, s1, s2, event, seg, Filter.ALL, 1d, "ref",
...
//      submit.submit(entry.getKey(), entry.getValue(), Submit.SubmitQueryType.PLAN);
  ...
  @Test
  public void testGenerateLogicalPlan() throws ParseException, PlanException, SegmentException, JsonProcessingException {
    String pID = "age";
    String[] segmentPool = new String[]{
            "{\"identifier\":[{\"op\":\"eq\",\"expr\":\"android.global.s77\",\"type\":\"CONST\"}]}",
            "{\"abflag\":[{\"op\":\"eq\",\"expr\":\"NewUIBarrack_NewNewbie_HeroHeadZoom_TroopAddConfig_B\",\"type\":\"CONST\"}]}",
    ...
    FormulaQueryDescriptor desc = new CommonFormulaQueryDescriptor(pID, beginDate, endDate, eventsPool[0], segmentPool[2], Filter.ALL,
            1d, intervalPool[1], CommonQueryType.NORMAL);
